    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div class="stat-card">
            ...
            <div>
                ...
                <div class="font-bold text-xl text-slate-800 mt-0.5">{{ $stats['total_agendas'] ?? 0 }}</div>
            </div>
        </div>
        ...
        <div class="stat-card">
            ...
            <div>
                ...
                <div class="font-bold text-xl text-amber-600 mt-0.5">{{ $stats['ongoing_agendas'] ?? 0 }}</div>
            </div>
        </div>
        ...
        <div class="stat-card">
            ...
            <div>
                ...
                <div class="font-bold text-xl text-blue-600 mt-0.5">{{ $stats['upcoming_agendas'] ?? 0 }}</div>
            </div>
        </div>
        ...
        @if($user->isAdministrator())
            <div class="stat-card">
                ...
                <div>
                    ...
                    <div class="font-bold text-xl text-purple-700 mt-0.5">{{ $stats['total_users'] ?? 0 }}</div>
                </div>
            </div>
        @elseif($user->isAdmin())
            <div class="stat-card">
        ...
        @else
            <div class="stat-card">
                ...
                <div>
                    ...
                    <div class="font-bold text-xl text-emerald-600 mt-0.5">{{ $stats['my_attendances'] ?? 0 }} Kali</div>
                </div>
            </div>
        @endif
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="panel">
                <div class="toolbar">
                    <div class="flex items-center gap-2">
                        ...
                        <h3 class="font-bold text-slate-800 text-sm">Agenda Rapat Terkini</h3>
                    </div>
                    <span class="text-xs text-slate-500">Menampilkan agenda terdekat</span>
                </div>
                ...
                <div class="p-4 sm:p-5 divide-y divide-slate-100">
                    @forelse($activeAgendas as $agenda)
                        <div class="py-4 first:pt-0 last:pb-0 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                            <div class="space-y-1">
                                <div class="flex items-center gap-2">
                                    <span class="status text-xs font-semibold {{ $agenda->status === 'ongoing' ? 'ongoing' : 'upcoming' }}">
                                        {{ $agenda->status === 'ongoing' ? 'Sedang Berlangsung' : 'Terjadwal' }}
                                    </span>
                                    <span class="text-xs text-slate-400">&bull;</span>
                                    <span class="text-xs uppercase font-mono font-medium text-slate-500">{{ $agenda->tipe_rapat }}</span>
                                </div>
                                ...
                                <div class="text-xs text-slate-500 flex flex-wrap items-center gap-3">
                                    ...
                                    <span class="inline-flex items-center gap-1.5">
                                        ...
                                        <span>{{ $agenda->lokasi_ruang ?? 'Daring / Virtual' }}</span>
                                    </span>
                                </div>
                            </div>
                            ...
                            <div class="shrink-0 flex items-center gap-2">
                                @if($agenda->status === 'ongoing')
                                    ...
                                    @else
                                        <a href="{{ route('attendances.create', $agenda) }}" class="button small flex items-center gap-1.5 text-xs bg-emerald-600 hover:bg-emerald-700 shadow-sm text-white">
                                    ...
                                    @endif
                                @else
                                    <span class="text-xs text-slate-400 italic">Presensi belum dibuka</span>
                                @endif
                            </div>
                        </div>
                    @empty
                        <div class="text-center py-8 text-slate-400 text-xs">
                            <svg class="mx-auto mb-2 text-slate-300" viewBox="0 0 24 24" width="36" height="36" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><rect width="18" height="18" x="3" y="4" rx="2" ry="2"/><line x1="16" x2="16" y1="2" y2="6"/><line x1="8" x2="8" y1="2" y2="6"/><line x1="3" x2="21" y1="10" y2="10"/></svg>
        ...
        <div class="space-y-4">
            <div class="panel">
                <div class="toolbar">
                    <div class="flex items-center gap-2">
                        <svg class="text-indigo-600" viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 20h9"/><path d="M16.5 3.5a2.12 2.12 0 0 1 3 3L7 19l-4 1 1-4Z"/></svg>
                        <h3 class="font-bold text-slate-800 text-sm">Log Aktivitas Terbaru</h3>
                    </div>
                </div>
                    ...
                    @forelse($recentLogs as $log)
                        <div class="py-3 first:pt-0 last:pb-0 space-y-1">
                            <div class="flex items-center justify-between text-[11px]">
                                <span class="font-semibold text-slate-800">{{ $log->user?->name ?? 'Sistem' }}</span>
                                <span class="text-slate-400 font-mono">{{ $log->created_at->diffForHumans() }}</span>
                            </div>
                            <p class="text-slate-600 leading-relaxed">{{ $log->description }}</p>
                            <div class="flex items-center gap-2 text-[10px] text-slate-400 font-mono">
                                <span>{{ $log->activity_type }}</span> &bull; <span>{{ $log->ip_address }}</span>
                            </div>
                        </div>
                    @empty
