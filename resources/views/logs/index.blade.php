    ...
    <div class="bg-white p-4 sm:p-5 rounded-2xl border border-slate-200 shadow-xs">
        <form method="GET" action="{{ route('admin.logs.index') }}" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-3 items-end">
            ...
            <div class="space-y-1 sm:col-span-2">
                <label for="search" class="text-xs font-semibold text-slate-700 block">Pencarian Kata Kunci</label>
                <div class="relative">
                    <input 
                        type="text" 
                        id="search" 
                    ...
                    >
                    <svg class="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" viewBox="0 0 24 24" width="15" height="15" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.3-4.3"/></svg>
                </div>
            </div>

            <!-- Tipe Aktivitas -->
            <div class="space-y-1">
                <label for="type" class="text-xs font-semibold text-slate-700 block">Tipe Aktivitas</label>
                <select id="type" name="type" class="input w-full text-xs">
            ...
            <div class="space-y-1">
                <label for="user_id" class="text-xs font-semibold text-slate-700 block">Aktor Pegawai</label>
                <select id="user_id" name="user_id" class="input w-full text-xs">
            ...
            <!-- Filter Actions Buttons (Full row or end column) -->
            <div class="sm:col-span-2 lg:col-span-6 flex items-center justify-end gap-2 pt-2 border-t border-slate-100">
                @if(request()->hasAny(['search', 'type', 'user_id', 'start_date', 'end_date']))
                    <a href="{{ route('admin.logs.index') }}" class="button small secondary text-xs">Reset Filter</a>
                @endif
                <button type="submit" class="button small text-xs font-semibold px-4">Terapkan Filter</button>
            </div>
        </form>
    </div>
    ...
    <div class="panel">
        <div class="toolbar">
            <div class="flex items-center gap-2">
                ...
                <h3 class="font-bold text-slate-800 text-sm">Riwayat Aktivitas & Jejak Audit</h3>
            </div>
        </div>
                            ...
                            <td>
                                @php
                                    $badgeClass = match(true) {
                                        ...
                                        str_contains($log->activity_type, 'LOGIN') || str_contains($log->activity_type, 'ATTENDANCE') => 'bg-emerald-50 text-emerald-700 border-emerald-200',
                                        str_contains($log->activity_type, 'EXPORT') => 'bg-indigo-50 text-indigo-700 border-indigo-200',
                                ...
                                @if($log->properties && count($log->properties) > 0)
                                    <div class="mt-1">
                                        <button 
                                            ...
                                            class="inline-flex items-center gap-1 text-[11px] text-blue-600 hover:text-blue-800 font-semibold underline"
                                        >
                                        ...
                                        <div id="payload-data-{{ $log->id }}" class="hidden">
                                            <div class="space-y-2 text-left">
                                                <div class="text-xs font-mono font-bold text-slate-500 uppercase">Tipe: {{ $log->activity_type }}</div>
                                                <div class="text-xs text-slate-700 mb-2">{{ $log->description }}</div>
                                                <pre class="bg-slate-900 text-slate-100 p-3 rounded-lg text-[11px] font-mono overflow-x-auto max-h-60 leading-relaxed">{{ json_encode($log->properties, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE) }}</pre>
                                            </div>
                                        </div>
                                    </div>
                                @endif
                        ...
                        <tr>
                            <td colspan="6" class="empty-search py-10">
                                <div class="space-y-2 text-center">
                                    <p class="text-slate-500 font-medium text-xs">Tidak ada rekaman log aktivitas yang sesuai dengan kriteria filter.</p>
                                    @if(request()->hasAny(['search', 'type', 'user_id', 'start_date', 'end_date']))
                                        <a href="{{ route('admin.logs.index') }}" class="button small secondary inline-flex text-xs">
