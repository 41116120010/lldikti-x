...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
        ...
        <div class="panel p-6 sm:p-8">
            <form method="POST" action="{{ route('admin.agendas.update-notulen', $agenda) }}" enctype="multipart/form-data" class="space-y-6">
                ...
                <div class="field">
                    ...
                    <p class="text-[11px] text-slate-400 mt-1">Uraikan jalannya rapat, dinamika diskusi, dan tanggapan peserta.</p>
                    @error('notulensi')
                ...
                <div class="space-y-3 pt-4 border-t border-slate-100">
                    <h3 class="text-xs font-bold uppercase tracking-wider text-slate-400">Tambah Foto Dokumentasi Kegiatan</h3>

                    <div class="field">
                        <label for="photos">Pilih Berkas Foto (Mendukung Multi-Upload)</label>
                        <input 
                            type="file" 
                            id="photos" 
                            name="photos[]" 
                ...
                <div class="flex items-center justify-end gap-3 pt-5 border-t border-slate-200">
                    <a href="{{ route('admin.agendas.show', $agenda) }}" class="button secondary text-xs">Kembali</a>
    ...
    <div class="lg:col-span-1 space-y-6">
        ...
        <div class="panel p-5 space-y-3">
            <h3 class="text-xs font-bold uppercase tracking-wider text-slate-400">Informasi Agenda</h3>
            <div class="space-y-2 text-xs">
                <div>
                    <span class="text-slate-400 block text-[11px]">Judul Rapat:</span>
                    <strong class="text-slate-800">{{ $agenda->judul_rapat }}</strong>
                </div>
                <div>
                    <span class="text-slate-400 block text-[11px]">Waktu:</span>
                    <span class="font-mono text-slate-700">{{ $agenda->waktu_mulai->translatedFormat('d M Y, H:i') }} WIB</span>
                </div>
                <div>
                    <span class="text-slate-400 block text-[11px]">Format & Lokasi:</span>
                    <span class="uppercase font-mono text-slate-700">{{ $agenda->tipe_rapat }}</span> &bull; 
                    <span class="text-slate-600">{{ $agenda->lokasi_ruang ?? 'Daring / Ruang Virtual' }}</span>
                </div>
                <div>
                    <span class="text-slate-400 block text-[11px]">Status Rapat:</span>
        ...
        <div class="panel p-5">
            <div class="flex items-center justify-between mb-3">
                <h3 class="text-xs font-bold uppercase tracking-wider text-slate-400">Dokumentasi Tersimpan</h3>
                <span class="text-xs font-mono text-slate-500">{{ $agenda->documentations->count() }} Foto</span>
            </div>
                ...
                <div class="grid grid-cols-2 gap-3">
                    @foreach($agenda->documentations as $doc)
                        <div class="relative group bg-slate-100 rounded-xl overflow-hidden border border-slate-200">
                            ...
                            <div class="p-1.5 bg-white text-[10px] text-slate-600 truncate border-t border-slate-100">
                            ...
                            >
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="w-6 h-6 rounded-full bg-rose-600 text-white flex items-center justify-center shadow-md hover:bg-rose-700" title="Hapus Foto">
            ...
            @else
                <div class="p-6 text-center bg-slate-50 rounded-xl border border-dashed border-slate-200 text-xs text-slate-400">
